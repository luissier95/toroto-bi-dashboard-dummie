...
# Filtro QA (Outliers) - Solo montos válidos
...
for mes in meses_proyectar:
    np.random.seed(42) # Para que la proyección sea estable al recargar
...
# ==========================================
# 4. CÁLCULOS DINÁMICOS (CORREGIDOS)
# ==========================================
# Creamos la tabla dinámica de OCs
a_pivot = pd.pivot_table(ocp, index='Rubro', columns=['anio', 'month'], values='Monto (MXN)', aggfunc='sum').fillna(0)

# Generar nombres de columnas de forma segura para evitar KeyErrors
nuevas_cols = []
...
# Blindaje: Si el mes seleccionado no existe en los datos, se crea con ceros
...
# Lógica de Amortización y Seguimiento
...
with tab2:
    ...
    st.dataframe(seguimiento.drop(columns=['Liquidación']).style.format("${:,.0f}"), use_container_width=True)

with tab3:
    st.write("### Planificación Presupuestal Anual (Proyectada)")
    st.write("Esta tabla contiene los montos asignados originalmente y las proyecciones para el resto del año:")
    
    cols_psto = [c for c in psto.columns if '- PSTO' in c]
    st.dataframe(
        psto[['Rubro'] + cols_psto].set_index('Rubro').style.format("${:,.0f}"), 
